...
const filteredSchedules = computed(() => {
  ...
  if (semesterFilter.value !== 'all') filtered = filtered.filter(s => s.semester === semesterFilter.value);
  if (dayFilter.value !== 'all') filtered = filtered.filter(s => s.day === dayFilter.value);
    ...
    <div class="p-8 space-y-6">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
        <div v-for="(stat, idx) in [
          { label: 'Mata Kuliah', val: stats.totalMK, icon: BookOpen, color: 'text-indigo-500' },
          { label: 'Dosen Pengajar', val: stats.totalDosen, icon: Users, color: 'text-emerald-500' },
          { label: 'Hari Aktif', val: stats.activeDays, icon: Calendar, color: 'text-sky-500' }
        ]" :key="idx" class="bg-white/40 dark:bg-white/5 backdrop-blur-md border border-white/20 dark:border-white/10 p-5 rounded-2xl flex justify-between items-center shadow-soft">
          <div>
            <p class="text-[10px] font-black text-primary-400 uppercase tracking-widest">{{ stat.label }}</p>
            <p class="text-2xl font-black text-primary-900 dark:text-white">{{ stat.val }}</p>
          </div>
          <component :is="stat.icon" :class="stat.color" class="w-10 h-10 bg-white/10 p-2 rounded-xl" />
        </div>
      </div>
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-left border-collapse">
            ...
            <tr class="bg-primary-50/50 dark:bg-white/5 border-b border-primary-100 dark:border-white/10 text-[10px] font-black text-primary-400 uppercase tracking-widest">
              ...
              <th v-if="isAdmin" class="p-5 text-center">Aksi</th>
            </tr>
            </thead>
            <tbody class="divide-y divide-primary-100 dark:divide-white/5">
            <tr v-if="loading" v-for="i in 3" :key="i" class="animate-pulse">
              <td :colspan="isAdmin ? 7 : 6" class="p-8"></td>
            </tr>
        ...
        <div class="p-8 space-y-6">
          ...
          <form @submit.prevent="handleSubmit" class="space-y-4">
            ...
            <div class="space-y-1">
              ...
              <input v-model="form.courseName" required placeholder="Contoh: Pemrograman Dasar" class="w-full bg-primary-50 dark:bg-white/5 border border-primary-100 dark:border-white/10 rounded-2xl px-5 py-3 text-xs font-bold dark:text-white outline-none focus:ring-2 focus:ring-secondary-500" />
            </div>

            <div class="space-y-1">
              <label class="text-[9px] font-black text-primary-400 uppercase ml-2">Dosen Pengajar</label>
              <input v-model="form.lecturer" required placeholder="Nama Lengkap Dosen" class="w-full bg-primary-50 dark:bg-white/5 border border-primary-100 dark:border-white/10 rounded-2xl px-5 py-3 text-xs font-bold dark:text-white outline-none focus:ring-2 focus:ring-secondary-500" />
            </div>

            <div class="space-y-1">
              <label class="text-[9px] font-black text-primary-400 uppercase ml-2">Ruangan</label>
              <input v-model="form.room" required placeholder="Contoh: Lab Komputer 1 / R.302" class="w-full bg-primary-50 dark:bg-white/5 border border-primary-100 dark:border-white/10 rounded-2xl px-5 py-3 text-xs font-bold dark:text-white outline-none focus:ring-2 focus:ring-secondary-500" />
            </div>
